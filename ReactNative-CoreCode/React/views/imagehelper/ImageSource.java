...
public class ImageSource {
  ...
  private Uri computeUri(Context context) {
    try {
      ...
      Log.d("zakj-rn", "compute uri = " + uri.toString());

      //added by hufuyi 2018/05/23
      if (uri.getScheme() == null){
        Uri temp = ZAKJResourceHelper.instance.computeLocalUri(mSource, context);

        if (temp != null){
          mSource = temp.toString();
          uri = temp;
        }
      }
      //end
